...
% set up the right hand side of the ODE
f = @(t,y) y * tan(t) + sin(t);
...
% time we are integrating to
t1 = pi;

%  ode45 for exact solution
soln = ode45(f, [t0, t1], y0);
...
% specific solution : y = - (cos(t)/2)

% exact solution
tt = linspace(t0, t1, 50);
yy = - (cos(tt)/2);
...
% computing interpolated values at 10 grid points between 2 and 3
...
% find pointwise error
pointwise_err = abs(yexact - soln.y);
disp(pointwise_err);
fprintf("Maximum ERROR: %g \n", max(pointwise_err));

% visualization of error
semilogy(soln.x, pointwise_err, 'LineWidth', 3);
xlabel('t');
ylabel('error');

% The error is the largest when t = 1.559 which is equal to pi/2
% The reason why the error is largest is because the rate of change in derivative is the highest
% since tan(t) approaches infinity as t approaches pi/2. ode24 is
% inaccurate near discontinuity(vertical asymptote)
%% Exercise 4
...
% set up the right hand of the ODE
f = @(t,y) 1/(y^2);
...
tt = linspace(1, 10, 12);
yy_part1 = 3*tt-2;
yy_part2 = nthroot(yy_part1, 3); 

% plot
plot(tt, yy_part2, soln.x, soln.y, 'x', 'MarkerSize',10, 'LineWidth', 2);
...
% using ode45 to solve the equation
soln = ode45(f, [t0, t1], y0);
...
xlabel('t');
ylabel('y');
title('Exercise 6 : Step size reduced error')


% The warning says to maintain accuracy as y' grows rapidly (due to y^3), 
% the step size must be reduced. As the
% derivative grows larger and larger, step size becomes too small, 
% preventing the solver from progressing as the solution approaches infinity before t=1.
...
% define variables using symbolic
syms t s y;

% define f
f = 1 + y^2

phi = [sym(0)]; % initally approaximnate phi = 0
...
for i = 1:N
    ...
    func = subs(f, y, phi(i)); % = function to integrate y from previous phi
    func = subs(func, t, s); %   t to s : change variable
    newphi = int(func, s, 0, t) + 1; % integrate to find next approximation 
    phi = cat(2, phi, [newphi]); % update previous approximations by adding new phi

end

% plot 
picard = ezplot(phi(N + 1), [0,1.25]);
set(picard,'Color','green');

% general solution : y = tan(t + C)
% specific solution for y(0) = 1 is: y = tan(t + (pi/4))

hold on;
exact = ezplot(tan(t + (pi/4)), [0,1.25]);
hold off;
xlabel('t');
ylabel('y');
title('Picard Approximations');

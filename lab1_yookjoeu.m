...
clear y; %clear y and preallocate y again
%output : Elapsed time is 0.129347 seconds.

tic; %start timing -> vertorization
y = atan(x(j));
toc;
clear y;
%output : Elapsed time is 0.002941 seconds.
...
%General solution of the ODE is determined through integrating factor
% y = (t+c)/(exp(t^2)
% c is computed by plugging in the solutions for initial conditions.
% As a result, y = (t-e-1)/exp(t^2) when y(1) = -1
%y = (t-1)/exp(t^2) when y(1) = 0
%y = (t+e-1)/exp(t^2) when y(1) = 1

t = linspace(1,5,100);
y0 = (t-exp(1)-1)./exp(t.^2);
y1 = (t - 1) ./ exp(t.^2);
y2 = (t + exp(1) -1) ./ exp(t.^2);
plot(t,y0,t,y1,t,y2);
title("Exercise 5");
legend("y0 : y(1) = -1 ", "y1 : y(1) = 0", "y3 = y(1) = 1");
...
x = linspace(-5,5,100);
%plot
plot(x, f(x),'r-','LineStyle','--');
xlabel('x');
ylabel('f(x)');
title('f(x) = (x^3 - abs(x)) * ln(x^2 + 1)')
...
% write function using vectorization in my definition
f = @(x,y) y + y.^3 - cos(x) + x.^2 -x.^4;

% Compute each values @provided points 
x = 0; y = 0;
fprintf("f(0,0):")
fprintf(' f(x = %g, y = %g) = %g\n', x, y, f(x, y));
x = pi/2;
...
% define the function  
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4
x_0 = 0; %solution at x=0

guess = 1; %random guess
g = @(y) f(x_0, y);
y = fzero(g, guess);
fprintf('first guess : y(%g) = %g\n', x_0, y);

guess = 10; %check if y(0) is the same for all guesses
y = fzero(g, guess);
fprintf('second guess: y(%g) = %g\n', x_0, y);

guess = 50;
y = fzero(g, guess);
fprintf(' third guess :y(%g) = %g\n', x_0, y);

guess = 0.211;
y = fzero(g, guess);
fprintf(' last guess : y(%g) = %g\n', x_0, y);
%for all guesses, y(0) = 0.682328

fprintf('Solution @ x = 0 is y(%g) = %g\n',x_0, y);

% Find two solutions of y_0
y_0 = 1/2;
g2 = @(x) f(x,y_0);
...
fprintf("First solution @ y = 1/2 is x(%g) = %g\n", y_0, x)


%guess = 0.001;    this results in NaN or Inf encounter
%x = fzero(g2, guess);
%fprintf('x(%g) = %g\n', y_0, x);
%guess = 50;           this results in NaN
%x = fzero(g2, guess);
%fprintf('x(%g) = %g\n', y_0, x);
...
fprintf("Second solution @ y = 1/2 is x(%g) = %g\n", y_0, x);
...
%from the original function, "+1" is required to make the function passes
...
for j = 1:length(x_val)
    y_val(j) = fzero(@(y) + f(x_val(j), y), guess);
end
...
% then, sin(x) + x.^3 - ln(abs(x)) = 0    note :"log" is ln(natural log) in matlab
% ln(abs(y)) = -sin(x) + x^3 + C
% c=0 from y(0) =1 
...
% 100 grid 
x_val = linspace(-1.5,1.25, 100);
y_val = zeros(size(xvals)); 

guess = 0.5;

for i = 1:length(x_val)
   y_val(i) = fzero(@(y) f(x_val(i), y), guess);
end 
...
xlabel("x");
ylabel("y");
title("Exercise 10: Solve dy/dx = ( -cos x + 3x^2 ) y given initial condition");
...
% ANSWER
% 
% As x approaches infinity, y approaches zero given that the slope lines(dy/dx) 
% are approaching zero as the x increases in both positively and negatively.
% 
% The soultions are symmetric about the y axis because because the sign(positive/negative) 
% of the solution is determined by x only as y^2 is always positive. For given 
% positve or negative x value at each point, the | |dy/dx|| (the absoulte value 
% of the slope) is the same, but the change in sign results in the symmetry about 
% the y axis
%% Exercise 12
...
% ANSWER
%% 
% # y0<−1 : Solution increases as x→inf, approaching y→−1.  Behavior for x→−inf⁡x 
% is unknown.
% # −1≤ y0 <1 : Solution increases, approaching y→1 as x→inf⁡, and y→−1 as x→−inf⁡.
% # 1< y0 ≤ 2: Solution decreases, approaching y→1 as x→inf⁡ , and y→2  as x→−inf⁡.
% # y0>2 : Solution increases with unknown behavior as x→inf⁡, approaching y→2 
% as x→−inf⁡x \to -\infx→−inf.
%% 
% Solution can't cross because crossing implies different behavior (phase portraits) 
% for the same initial condition, which does not abide by the uniqueness property 
% of differential equations.
% 
% The problem with numerical methods represented from the crossing is caused 
% by the accumulated errorsfrom the step size. Euler only accounts for the first 
% derivative, while Runge-Kutta considers the first four, but both methods still 
% have errors over time.
% 
% 
